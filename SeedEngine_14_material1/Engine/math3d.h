...
namespace Seed
{
	...
	public:
		...
		vector4f(const Vector<float, 4>& v) {
			for (unsigned i = 0; i < 4; ++i)
				vec[i] = v[i];
		}
		inline vector4f& operator=(const Vector<float, 4>& v) {
			for (unsigned i = 0; i < 4; ++i)
				vec[i] = v[i];
			return *this;
		}
	...
	public:
		...
		inline matrix4f& initProjection(float fov, float ratio, float zNear, float zFar)
		{
			...
			m[0][0] = cotHalfFOV; m[0][1] = 0;								  m[0][2] = 0;			  m[0][3] = 0;
			m[1][0] = 0;					m[1][1] = cotHalfFOV * ratio;	m[1][2] = 0;				m[1][3] = 0;
