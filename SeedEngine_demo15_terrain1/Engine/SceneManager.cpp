...
namespace Seed
{
	SceneManager::SceneManager() {
		...
		for (u32 i = 0; i < df_N; ++i)
		{
			if (i == df_depth)
				...
				(renderingEngine->getWidth(), renderingEngine->getHeight(), PF_R32);
			else
				mDeferedRT[i] = renderingEngine->createRenderTarget
				(renderingEngine->getWidth(), renderingEngine->getHeight(), PF_A8R8G8B8);
		}
	...
	void SceneManager::renderOneFrame(vector<GameObject*> *q)
	{
		...
		if (0) {
			...
			if (mActiveShader == normalMapShader) {
	...
	void SceneManager::_renderOneFrame(vector<GameObject*> *q)
	{
		Window *w = RenderingEngine::getInstance().getWin32();
		float x = w->getMouseX();
		float y = w->getMouseY();
		float z = 1.0f;
		x = (x / w->getWidth() * 2 - 1) / mainCamera->getProjection()[0][0];
		y = (-(y / w->getHeight() * 2 - 1)) / mainCamera->getProjection()[1][1];

		vector3f pickpoint(x, y, z);
		vector3f eye = mainCamera->getEye();

		vector3f ray = mainCamera->getView().inverse().transformNormal(pickpoint);
		Log << ray.getX() << ' ' << ray.getY() << ' ' << ray.getZ() << endl;
		vector3f viewdir =  mainCamera->getForward();
		Log << viewdir.getX() << ' ' << viewdir.getY() << ' ' << viewdir.getZ() << endl;

		(*q)[1]->setPos(mainCamera->getPos() + ray * 20)->updateTransform();

			////-------------xxxxoooo--------------------------------------
			u32 i = 0;
			Material *shadow = mActiveLight->getShadow()->getMaterial();
			shadow->applyMaterial(0, 0, 1, 1);
			for (i=0; i < q->size() - 1; ++i) {
				(*q)[i]->addLight(mActiveLight);
				(*q)[i]->renderShadow(shadow);
			}
			...
			for (i = 0; i < q->size() - 1; ++i) {
					(*q)[i]->_render(mainCamera);
			}
			////-----------xxxxxooooo--------------------------------
			//
			//RenderingEngine::getInstance().drawLine(vector3f(2, 0, 0), vector3f(2, 5, 0));
	}
