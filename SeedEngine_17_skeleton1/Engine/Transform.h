...
namespace Seed
{
	...
	public:
		...
		// 局部坐标系transform			 
		virtual inline matrix4f getTransform(TransformType type = TS_parent) const 
		{
			switch (type) {
		...
		virtual inline const vector3f& getPos(TransformType type = TS_parent) const
		{
			switch (type) {
		...
		virtual inline const Quaternion& getRot(TransformType type = TS_parent) const
		{
			switch (type) {
		...
		virtual inline const vector3f& getScale(TransformType type = TS_parent) const
		{
			switch (type) {
		...
		// 设为未更新
		virtual inline void needUpdate() {
			mSelfNeedUpdate = true;
			callFaUpdate();
		}

		virtual inline void callFaUpdate() {
			if (parent) {
		...
		virtual inline void calTransform()
		{
		...
		virtual inline void calFinalTransform()
		{
			matrix4f translate, rotate, scale;
		...
		virtual inline void updateTransform() {
		...
		virtual inline matrix4f getFinalInvTransform() const {
