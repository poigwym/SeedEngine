...
public:
	...
	void init() {
		...
		//dfmtl = MaterialManager::getInstance().createMaterial("skeleton.material");

		bonemtl = MaterialManager::getInstance().createMaterial("skeleton.material");
		...
		engine->getSceneManager()->getRoot()->addObject(ts1);
		engine->getSceneManager()->getRoot()->addObject(bone);
	...
	void render()
	{
		...
		//dfmtl->applyMaterial(ts1, camera, 1, 1);
		//ts1->_render(camera);


		bonemtl->applyMaterial(bone, camera, 1, 1);
		bone->_render(camera);
