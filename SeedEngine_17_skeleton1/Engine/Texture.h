...
namespace Seed
{
	...
	protected:
		u32 mWidth, mHeight;
		u32 mMip;														// mip
		PixelFormat mFmt;										// pixelformat


	public:
		
		// @ Texture() 
		// desc : no feather, no memory
		Texture() {}

		// @ Texture(u32 mWidth, u32 mHeight, PixelFormat fmt, u32 mip) 
		// desc : only set feather to texture, create a empty texture
		Texture(u32 mWidth, u32 mHeight, PixelFormat fmt, u32 mip) :
		...
		// @createTexture : 
		// desc:  feather is known,  create a empty texture of load from file.
		virtual Texture* createTexture(const string &file = "") = 0;

		// @createRenderTarget 
		// desc:  feather is known, create a RT.
		virtual Texture* createRenderTarget() = 0;

		// @createFromFile
		// desc : don't know feather, load from file directly.
		virtual Texture* createFromFile(const string &file) = 0;

		// @bindAsRenderTarget
		// desc : 
		// index: index of RT
		// clearColor to clear background color 
		// clearDS to clear depth-stencil of the RT
