...
namespace NBengine
{
		...
		hInstance(hInstance), hwnd(hwnd)
	{
		...
		for (u32 i = 0; i < mMaxRT; ++i) {
			d3d9Texture *back = new d3d9Texture(mDevice, desc.Width, desc.Height, PXFMT_A8R8G8B8, 1, TEXF_LINEAR);
	...
	Texture* d3d9RenderingEngine::createRenderTarget(u32 w, u32 h, ColorFormat fmt)
	{
		d3d9Texture *r = (d3d9Texture*)TextureManager::getInstancePtr()->create(w, h, fmt, 1, TEXF_LINEAR);
