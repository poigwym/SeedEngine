...
namespace NBengine
{
	d3d9Texture::d3d9Texture(LPDIRECT3DDEVICE9 device, u32 w, u32 h, ColorFormat fmt, u32 mip, FilterType filter, const string &file):
		device(device), Texture(w, h, fmt, mip, filter) {
	...
	Texture* d3d9Texture::load(const string &file)
	{
		...
		if (FAILED(D3DXCreateTextureFromFileEx(device, file.c_str(),
			...
			d3d9Mappings::getFilterType(mFiltertype), d3d9Mappings::getFilterType(mFiltertype),
